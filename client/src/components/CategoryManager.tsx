...
export const CategoryManager: React.FC<CategoryManagerProps> = ({
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error instanceof Error) alert("Failed to create category");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      if (error instanceof Error) alert("Failed to delete category");
    }
  };
        ...
        <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
          {categories.length === 0 ? (
            ...
            categories.map((cat) => (
              <div
                key={cat.category_id}
                className="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg group hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
              >
